...
Version = 'PID-Analyzer-withUI 0.1'
...
class Ui_MainWindow(object):
    ...
    def start(self): # start
        ...
        path = self.lineEdit.text() # get path
        plot_name = self.lineEdit_2.text() # get plot_name
        if noise_bounds == 'auto': # get noise_bounds
            pass
        else:
        ...
        if not os.path.isfile(blackbox_decode):
            ...
            self.textBrowser.append('Decoding with %r' % blackbox_decode_path)
            self.textBrowser.append(Version)
            #self.textBrowser.append('你好啊QvQ')

        if not os.path.isdir(plot_name): # test the plot_name
            plot_name = os.path.join(os.getcwd(), 'tmp')
            try:
        ...
        if os.path.isfile(path) and os.path.isfile(blackbox_decode) and os.path.isdir(plot_name): #  test the path
            try:
...
class Thread_1(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            test = BB_log(path, os.path.relpath(plot_name, os.getcwd()), blackbox_decode, show, noise_bounds)
            if show == 'Y':
                logging.info('Analysis complete, showing plot.')
                plt.cla()
                plt.clf()
                os.startfile(plot_name)
                #subprocess.call(["open", plot_name]) # for mac
            else:
                logging.info('Analysis complete, exit.')
                plt.cla()
                plt.clf()
            #print(path, os.path.relpath(plot_name, os.getcwd()), blackbox_decode, show, noise_bounds)
        except Exception as e:
            logging.error(e)
        qmut_1.unlock()
