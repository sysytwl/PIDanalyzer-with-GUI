...
Version = 'PID-Analyzer-withUI 0.2'
...
class CSV_log:
    ...
    def plot_all_noise(self, traces, lims): #style='fancy' gives 2d hist for response
        ...
        plt.savefig(self.file[:-13] + '_' + str(self.headdict['logNum'])+'_noise.png')
        return fig
    ...
    def plot_all_resp(self, traces, style='ra'): # style='raw' for response vs. time in color plot
        ...
        ax4.axis('off')
        logging.info('Saving as image...')
        plt.savefig(self.file[:-13] + '_' + str(self.headdict['logNum'])+'_response.png')
        return fig
...
class BB_log:
    def __init__(self, log_file_path, name, blackbox_decode, noise_bounds):
        self.blackbox_decode_bin_path = blackbox_decode
            #self.tmp_dir = os.path.join(os.path.dirname(log_file_path), name)
        self.tmp_dir = name
        if not os.path.isdir(self.tmp_dir):
            os.makedirs(self.tmp_dir)
        logging.basicConfig(filename = name + '/logging.log', format='%(levelname)s %(asctime)s %(filename)s:%(lineno)s: %(message)s', level=logging.DEBUG)
        self.name = name
...
#BB_log(clean_path("c:/Users/19577/Downloads/PID-Analyzer-0.52/good_tune.BBL"), clean_path("C:/Users/19577/Downloads/PID-Analyzer-0.52/tmp"), clean_path("C:/Users/19577/Downloads/PID-Analyzer-0.52/Blackbox_decode.exe"), "auto")