...
# Form implementation generated from reading ui file 'C:\Users\19577\Downloads\PID-Analyzer-0.52\untitled.ui'
#
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.checkBox.setText(_translate("MainWindow", "完成时打开图片"))
    ...
    '''
    def tmp(self,status): #auto or manual place for saving
        global plot_name
    ...
    def start(self): # start
        ...
        if noise_bounds == 'auto':
            ...
            print(noise_bounds, PIDAnalyzer.CSV_log.check_lims_list(self,noise_bounds))
        
        # define the blackbox decode path & test if it is exsit
        blackbox_decode = os.path.join(os.getcwd(), 'Blackbox_decode.exe')
        blackbox_decode_path = PIDAnalyzer.clean_path(blackbox_decode)
        if not os.path.isfile(blackbox_decode):
            self.textBrowser.append('Could not find "Blackbox_decode.exe". You may need to install it from ''https://github.com/cleanflight/blackbox-tools/releases.')
        else:
            self.textBrowser.append('Decoding with %r' % blackbox_decode_path)
            self.textBrowser.append(PIDAnalyzer.Version)
        ...
        print(os.path.isfile(path), os.path.isdir(plot_name))
        if os.path.isfile(path): #  test the path
            try:
...
class Thread_1(QThread):
    ...
    def run(self):
        ...
        qmut_1.lock()
        try:
            PIDAnalyzer.run_analysis(path, os.path.relpath(plot_name, os.getcwd()), blackbox_decode, show, noise_bounds)
